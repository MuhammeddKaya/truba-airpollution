...
def build_bilstm(neurons=50, dropout=0.2, optimizer='adam', h_layers=4):
    model = Sequential()
    model.add(Input(shape=(None, X_train_scaled.shape[1])))
    for i in range(h_layers):
        if i == h_layers - 1:  # Son LSTM katmanı
...
def train_and_evaluate(params):
    ...
    print(f"Training model with parameters: neurons={neurons}, dropout={dropout}, h_layers={h_layers}, optimizer={optimizer}, time_window={time_window}, batch_size={batch_size}")
    ...
    model = build_bilstm(neurons=neurons, dropout=dropout, optimizer=optimizer, h_layers=h_layers)
    ...
    with open("./bilstm_revision_results.txt", "a") as f:
        f.write(str({
            'neurons': neurons,
            'dropout': dropout,
            'h_layers': h_layers,
...
print("Results saved to 'bilstm_revision_results.txt'")