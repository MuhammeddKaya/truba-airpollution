...
def build_gru(neurons=50, dropout=0.2, optimizer='adam', h_layers=4):
    model = Sequential()
...
def train_and_evaluate(params):
    ...
    print(f"Training GRU model with parameters: neurons={neurons}, dropout={dropout}, h_layers={h_layers}, optimizer={optimizer}, time_window={time_window}, batch_size={batch_size}")
    ...
    model = build_gru(neurons=neurons, dropout=dropout, optimizer=optimizer, h_layers=h_layers)
    ...
    with open("./gru_revision_results.txt", "a") as f:
        f.write(str({
            'neurons': neurons,
            'dropout': dropout,
            'h_layers': h_layers,
...
print("Results saved to 'gru_revision_results.txt'")