...
from tensorflow.keras.layers import GRU, Bidirectional, Dense, Dropout, Input, LSTM
...
def build_cnn_lstm(neurons=50, dropout=0.2, optimizer='adam', conv_neurons=64, cnn_layers=1, lstm_layers=1):
    ...
    for i in range(lstm_layers):
        if i == lstm_layers - 1:  # Son LSTM katmanı
...
def train_and_evaluate(params):
    ...
    with open("cnn_lstm_revision.txt", "a") as f:
        f.write(str({
            'neurons': neurons,
            'dropout': dropout,
            'optimizer': optimizer,
...
neurons_list = [100, 200] # 2
dropout_list = [0.1, 0.2] # 2
optimizer_list = ['Adam','RMSprop'] # 2
time_window_sizes = [8, 24, 72] # 3
batch_size_list = [16, 32, 64] # 3
conv_neurons_list = [64, 128] # 2
cnn_layers_list = [1, 2] # 2
lstm_layers_list = [2, 3] # 2

param_combinations = list(product(neurons_list, dropout_list, optimizer_list, time_window_sizes, batch_size_list, conv_neurons_list, cnn_layers_list, lstm_layers_list))

# Paralel olarak modelleri eğit ve değerlendir
results = Parallel(n_jobs=8)(delayed(train_and_evaluate)(params) for params in param_combinations)

print("Results saved to 'cnn_lstm_revision.txt'")