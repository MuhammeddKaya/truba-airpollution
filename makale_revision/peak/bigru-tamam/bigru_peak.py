...
def build_bigru(neurons=50, dropout=0.2, optimizer='adam', h_layers=2):
    model = Sequential()
    model.add(Input(shape=(None, X_train_scaled.shape[1])))
    for i in range(h_layers):
        return_seq = i != h_layers - 1
        model.add(Bidirectional(GRU(neurons, activation='relu', return_sequences=return_seq)))
...
def train_and_evaluate(params):
    ...
    print(f"Training BiGRU: neurons={neurons}, dropout={dropout}, h_layers={h_layers}, optimizer={optimizer}, time_window={time_window}, batch_size={batch_size}")
    ...
    # Model eğitimi
    model = build_bigru(neurons, dropout, optimizer, h_layers)
    ...
    with open("bigru_zscore_peak_results.txt", "a") as f:
        f.write(str({
            'neurons': neurons,
            'dropout': dropout,
            'h_layers': h_layers,
...
print("Z-score tabanlı peak analizi tamamlandı. Sonuçlar 'bigru_zscore_peak_results.txt' dosyasına kaydedildi.")
