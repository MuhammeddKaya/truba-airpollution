...
def plot_last_21_days(model_result):
    ...
    plt.title('Actual and predicted values for PM2.5 (Last 21 days)')
    plt.xlabel('Date', fontsize=14)
    plt.ylabel('PM2.5 Concentration', fontsize=14)
    plt.legend()
    plt.grid(False)  # Grid çizgilerini kaldır
    plt.tight_layout()
    plt.savefig(f"pm25_last_21_days{model_result['r2']:.4f}--{model_result['r2_peak']:.4f}.png", dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"R² Score: {model_result['r2']:.4f}")
    print(f"R² Peak Score: {model_result['r2_peak']:.4f}")

# Paralel çalışacak fonksiyon
def train_attempt(params, target_r2, target_r2_peak, attempt_num):
    print(f"\nAttempt {attempt_num}")
...
batch_size = 4  # Paralel çalışacak işlem sayısı (genellikle CPU çekirdek sayısı)
success = False
best_result = None
...
for batch_start in range(0, max_attempts, batch_size):
    ...
    batch_results = Parallel(n_jobs=-1)(
        delayed(train_attempt)(params, target_r2, target_r2_peak, attempt_num) 
...
if success:
    print("\nTarget R² values reached!")
    plot_last_21_days(best_result)
else:
    print(f"\nNo model reached the target R² values after {max_attempts} attempts.")
    if best_result is not None:
        print(f"Best R² achieved: {best_result['r2']:.4f}")
        print(f"Best R² Peak achieved: {best_result['r2_peak']:.4f}")
    else:
        print("No successful model trained.")