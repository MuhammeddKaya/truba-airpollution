...
# BiLSTM modeli oluşturma
def build_bilstm(neurons=50, dropout=0.2, optimizer='adam', h_layers=2):
    model = Sequential()
    model.add(Input(shape=(None, X_train_scaled.shape[1])))
    for i in range(h_layers):
        return_seq = i != h_layers - 1
        model.add(Bidirectional(LSTM(neurons, activation='relu', return_sequences=return_seq)))
...
def train_and_evaluate(params):
    ...
    print(f"Training BiLSTM: neurons={neurons}, dropout={dropout}, h_layers={h_layers}, optimizer={optimizer}, time_window={time_window}, batch_size={batch_size}")
    ...
    # Model eğitimi
    model = build_bilstm(neurons, dropout, optimizer, h_layers)
    ...
    if np.any(np.isnan(y_true)) or np.any(np.isnan(y_pred_flat)) or len(y_true) != len(y_pred_flat):
        print(f"SKIP: NaN veya boyut uyuşmazlığı! params: {params}")
        with open("bilstm_zscore_peak_results.txt", "a") as f:
            f.write(str({
                'neurons': neurons,
                'dropout': dropout,
                'h_layers': h_layers,
                ...
                'batch_size': batch_size,
                'error': 'NaN veya boyut uyuşmazlığı'
            }) + "\n")
        return None
    ...
    mae = np.mean(np.abs(y_true - y_pred_flat))
    ...
    y_pred_peak = y_pred_flat[peak_mask]
...
print("Z-score tabanlı peak analizi tamamlandı. Sonuçlar 'bilstm_zscore_peak_results.txt' dosyasına kaydedildi.")
