...
def train_and_evaluate(params):
    ...
    mae = np.mean(np.abs(y_test[time_window:] - y_pred.flatten()))

    # Z-score ile dinamik peak analiz
    z_scores = (y_test[time_window:] - np.mean(y_test[time_window:])) / np.std(y_test[time_window:])
    z_threshold = 1.5
    peak_mask = z_scores > z_threshold
    y_true_peak = y_test[time_window:][peak_mask]
