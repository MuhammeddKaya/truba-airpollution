...
def build_lstm(neurons=50, dropout=0.2, optimizer='adam', h_layers=2):
    model = Sequential()
...
def train_and_evaluate(params):
    ...
    print(f"Training LSTM: neurons={neurons}, dropout={dropout}, h_layers={h_layers}, optimizer={optimizer}, time_window={time_window}, batch_size={batch_size}")
    ...
    # Model eğitimi
    model = build_lstm(neurons, dropout, optimizer, h_layers)
    ...
    with open("lstm_zscore_peak_results.txt", "a") as f:
        f.write(str({
            'neurons': neurons,
            'dropout': dropout,
            'h_layers': h_layers,
...
print("Z-score tabanlı peak analizi tamamlandı. Sonuçlar 'lstm_zscore_peak_results.txt' dosyasına kaydedildi.")
